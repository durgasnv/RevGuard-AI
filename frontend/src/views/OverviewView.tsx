    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <KpiCard
          ...
          sub={`${detectReport.failed_count} failed of ${detectReport.transactions_analyzed} txns`}
          tone="red"
        />
        <KpiCard
        ...
        <KpiCard
          ...
          sub={evaluation ? `${pct(evaluation.ai_strategy.recovery_rate, 2)} recovery rate` : ''}
          tone="green"
        />
        <KpiCard
          label="Unnecessary Interventions"
          value={evaluation ? evaluation.ai_strategy.unnecessary_interventions : '…'}
          sub={evaluation ? `baseline: ${evaluation.baseline.unnecessary_interventions}` : ''}
          tone="amber"
        />
      </div>
          ...
          {evaluation ? (
            <>
              ...
              <div className="mt-3 flex flex-wrap gap-4 border-t border-slate-800 pt-3 text-xs text-slate-400">
                <span>
                  Uplift:{' '}
                  <b className="num text-emerald-400">
                ...
                <span>
                  Waste avoided:{' '}
                  <b className="num text-emerald-400">
                    {evaluation.uplift.avoided_unnecessary_interventions}
                  </b>{' '}
                  interventions
                </span>
                <span>
                  Prevented hopeless actions:{' '}
                  <b className="num text-blue-300">
                    {evaluation.ai_strategy.prevented_interventions}
                  </b>
                </span>
              </div>
            </>
          ...
          <div className="space-y-1.5">
            {donut.map((d) => (
              <div key={d.name} className="flex items-center gap-2 text-xs text-slate-400">
                ...
                {d.name}
                <span className="num ml-auto font-medium text-slate-300">{inr(d.value)}</span>
              </div>
            ))}
          </div>
        </Card>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <Card
          ...
          right={
            <button onClick={goLeakage} className="text-xs text-blue-400 hover:text-blue-300">
              view all →
            </button>
          }
        >
              ...
              <div key={c.cluster_id} className="flex items-center gap-3 rounded-lg bg-slate-800/40 px-3 py-2">
                <span
                  className={`h-1.5 w-1.5 shrink-0 rounded-full ${
                    c.severity === 'high' ? 'bg-red-400' : 'bg-amber-400'
                  }`}
                />
                ...
                <span className="num text-xs font-semibold text-red-300">
